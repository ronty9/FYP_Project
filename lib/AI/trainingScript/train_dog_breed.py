...
def parse_args():
    parser = argparse.ArgumentParser(description="Train dog breed classifier with EfficientNetB0")
    parser.add_argument("--data_root", type=str, required=True, help="Folder containing train/val/test")
    parser.add_argument("--out_dir", type=str, default="/home/stfd/fyp_data/dog_train_effnet_12_18")
...
def build_model(img_size: int, num_classes: int):
    ...
    model = keras.Model(inputs, outputs, name="dog_breed_effnetb0")
...
def choose_best_model(out_dir: Path, val_ds):
    stage1_path = out_dir / "dog_breed_stage1_best.keras"
    stage2_path = out_dir / "dog_breed_stage2_best.keras"
...
def main():
    ...
    save_labels(class_names, out_dir, "dog_labels.json")
    ...
    callbacks_stage1 = make_callbacks(
        out_dir=out_dir,
        ckpt_name="dog_breed_stage1_best.keras",
    ...
    if args.epochs2 > 0:
        ...
        callbacks_stage2 = make_callbacks(
            out_dir=out_dir,
            ckpt_name="dog_breed_stage2_best.keras",
            csv_name="training_log.csv",
            append_csv=True,
        )
        hist2 = model.fit(
    ...
    final_model_path = out_dir / "dog_breed.keras"
    ...
    print("\nDone.")
    print(f"Model  : {out_dir / 'dog_breed.keras'}")
    print(f"Labels : {out_dir / 'dog_labels.json'}")
