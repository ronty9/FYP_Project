...
def parse_args():
    parser = argparse.ArgumentParser(description="Train cat/dog breed classifier with selectable backbone")
    parser.add_argument("--task", type=str, required=True, choices=["cat", "dog"])
    parser.add_argument(
        "--backbone",
        type=str,
        required=True,
        choices=["mobilenetv2", "resnet50v2", "efficientnetb0"],
    )
    ...
    parser.add_argument("--epochs2", type=int, default=6)
    parser.add_argument("--seed", type=int, default=42)
    parser.add_argument("--n_unfreeze", type=int, default=60)
...
def get_backbone(backbone_name: str, img_size: int):
    ...
    return base_model, dropout


def build_model(backbone_name: str, task: str, img_size: int, num_classes: int):
    ...
    base_model, dropout_rate = get_backbone(backbone_name, img_size)
    ...
    model_name = f"{task}_breed_{backbone_name.lower()}"
...
def save_labels(class_names, out_dir: Path, task: str):
    payload = {
        "class_names": class_names,
        "class_to_index": {name: idx for idx, name in enumerate(class_names)},
        "index_to_class": {str(idx): name for idx, name in enumerate(class_names)},
    }
    with open(out_dir / f"{task}_labels.json", "w", encoding="utf-8") as f:
...
def choose_best_model(out_dir: Path, val_ds, task: str):
    stage1_path = out_dir / f"{task}_breed_stage1_best.keras"
    stage2_path = out_dir / f"{task}_breed_stage2_best.keras"
...
def main():
    ...
    save_labels(class_names, out_dir, args.task)
    ...
    callbacks_stage1 = make_callbacks(
        out_dir=out_dir,
        ckpt_name=f"{args.task}_breed_stage1_best.keras",
    ...
    if args.epochs2 > 0:
        ...
        callbacks_stage2 = make_callbacks(
            out_dir=out_dir,
            ckpt_name=f"{args.task}_breed_stage2_best.keras",
            csv_name="training_log.csv",
            append_csv=True,
        )
        hist2 = model.fit(
    ...
    print("\n===== SELECT BEST MODEL =====")
    best_model, selection_info = choose_best_model(out_dir, val_ds, args.task)
    final_model_path = out_dir / f"{args.task}_breed.keras"
    ...
    print(f"Labels : {out_dir / f'{args.task}_labels.json'}")
