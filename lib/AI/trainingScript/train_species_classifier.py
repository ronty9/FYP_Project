...
def parse_args():
    parser = argparse.ArgumentParser(description="Train cat-vs-dog species classifier with EfficientNetB0")
    parser.add_argument("--data_root", type=str, required=True, help="Folder containing train/val/test")
    parser.add_argument("--out_dir", type=str, default="/home/stfd/fyp_data/species_train_effnet")
...
def build_datasets(data_root: Path, img_size: int, batch_size: int, ignore_decode_errors: bool):
    ...
    if len(class_names) != 2:
        raise ValueError(f"Species classifier expects exactly 2 classes, found: {class_names}")
...
def build_model(img_size: int):
    ...
    model = keras.Model(inputs, outputs, name="species_effnetb0")
...
def save_labels(class_names, out_dir: Path):
    payload = {
        "class_names": class_names,
        "class_to_index": {name: idx for idx, name in enumerate(class_names)},
        "index_to_class": {str(idx): name for idx, name in enumerate(class_names)},
    }
    with open(out_dir / "species_labels.json", "w", encoding="utf-8") as f:
...
def choose_best_model(out_dir: Path, val_ds):
    stage1_path = out_dir / "species_stage1_best.keras"
    stage2_path = out_dir / "species_stage2_best.keras"
...
def main():
    ...
    save_labels(class_names, out_dir)
    ...
    callbacks_stage1 = make_callbacks(
        out_dir=out_dir,
        ckpt_name="species_stage1_best.keras",
    ...
    if args.epochs2 > 0:
        ...
        callbacks_stage2 = make_callbacks(
            out_dir=out_dir,
            ckpt_name="species_stage2_best.keras",
            csv_name="training_log.csv",
            append_csv=True,
        )
        hist2 = model.fit(
    ...
    final_model_path = out_dir / "species_model.keras"
    ...
    print("\nDone.")
    print(f"Model  : {out_dir / 'species_model.keras'}")
    print(f"Labels : {out_dir / 'species_labels.json'}")
